...
def train():
    ...
    save_path = tl.make_path('save/unsupervised/{}/{}'.format(data_set, prior), verbose=False)
    ...
    encoder = Encoder(in_dim=x_dim, h_dim=1024, out_dim=z_dim, type='v1')
    decoder = Decoder(in_dim=z_dim, h_dim=1024, out_dim=x_dim, type='v1')
    ...
    d_fake = discriminator.feed_forward(z_fake)
    ...
    # optimizers
    A_solver = tf.train.AdamOptimizer(learn_rate).minimize(A_loss)
    D_solver = tf.train.AdamOptimizer(learn_rate).minimize(D_loss, var_list=dc_var)
    G_solver = tf.train.AdamOptimizer(learn_rate).minimize(G_loss, var_list=en_var)
    ...
    z_sample = tl.get_meshgrid(z_range=1)
